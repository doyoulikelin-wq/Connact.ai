...
2. Log in with **Google** or **Email + Password**
   - Private beta: use the landing page access gate to join the waitlist (email) or switch to invite code (or open `/#access`)
   - After submitting invite/waitlist on `/`, you’ll see a success/error banner and jump back to the gate
...
5. Auth / Accounts (recommended):
   ...
   # Optional: require invite gate before login (defaults to INVITE_ONLY)
   export INVITE_REQUIRED_FOR_LOGIN=true
   # Optional: switch landing variants (default: dark)
   export LANDING_VERSION=dark   # or substack
   ```
