"""AuthService unit tests (email verification + password / google + waitlist)."""
...
def test_password_signup_is_open_even_when_invite_only(tmp_path):
    """Invite-code gating has been removed; legacy invite_only kwarg is now a no-op."""
    service = AuthService(
...
def test_resend_verification_creates_new_token(tmp_path):
    ...
    first = service.create_password_user(email="c@example.com", password="password123")
...
def test_google_login_links_to_existing_password_user_by_verified_email(tmp_path):
    ...
    verification = service.create_password_user(
        email="d@example.com", password="password123"
    )
...
def test_profile_persistence_roundtrip(tmp_path):
    ...
    verification = service.create_password_user(email="e@example.com", password="password123")
    user_id = service.verify_email_token(verification.token)
    assert user_id
