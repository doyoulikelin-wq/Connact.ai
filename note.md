...
- 新增 Landing Page（`/`）：未登录展示产品介绍 + 入口（邀请码/Waitlist）
...
- 新增 landing page（`/`）：未登录用户先看到产品介绍和 access box（邀请码/Waitlist）。
...
- OpenAI 调用兼容性：开发者模式可选模型时，针对 GPT-5 base / o-series 这类不支持自定义采样参数的模型，在调用侧显式省略 `temperature` 等不兼容参数（不做运行时“自动学习”）。
